...
$(function () {
    ...
    $(".brand-block").each(function (index, block) {
        ...
        // Validate brand name
        if (trimmedName === "") {
            brandName.after('<small class="text-danger error-msg">Please enter brand name.</small>');
            isValid = false;
        } else if (trimmedName.length > 255) {
            brandName.after('<small class="text-danger error-msg">Max 255 characters allowed.</small>');
            isValid = false;
        } else {
            // Push uniqueness check promise
            ajaxChecks.push(
                $.post("/admin/brand/check-brand", { name: trimmedName }).then((response) => {
                    if (response.exists) {
                        brandName.after('<small class="text-danger error-msg">Brand name :'+trimmedName+' already exists.</small>');
                        isValid = false;
                    }
                })
            );
        ...
        if (trimmedDesc === "") {
            description.after('<small class="text-danger error-msg">Please enter description.</small>');
            isValid = false;
        } else if (trimmedDesc.length > 1000) {
            description.after('<small class="text-danger error-msg">Max 1000 characters allowed in description.</small>');
            isValid = false;
        }

        // Validate image
        if ($.trim(imageInput.val()) === "") {
            imageInput.after('<small class="text-danger error-msg">Please choose brand logo.</small>');
            isValid = false;
            ...
            if (!allowedTypes.includes(file.type)) {
                imageInput.after('<small class="text-danger error-msg">Invalid image format. Use jpg, jpeg, png, or webp.</small>');
                isValid = false;
            }

            if (file.size > 2 * 1024 * 1024) {
                imageInput.after('<small class="text-danger error-msg">Image size must be less than 2MB.</small>');
                isValid = false;
            }
        }
    });
